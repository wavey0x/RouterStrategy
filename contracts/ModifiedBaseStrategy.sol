...
abstract contract BaseStrategy {
    ...
    event OffsetLossOnFee(uint256 amount);
    ...
    address public stash = 0xE376e8e8E3B0793CD61C6F1283bA18548b726C2e;
    ...
    function harvest() external onlyKeepers {
        ...
        // Compute expected loss from fees. Must occur before the
        // vault report, which is where fees are taken.
        uint expectedLoss = lossChecker.check_loss(profit, loss);

        debtOutstanding = vault.report(profit, loss, debtPayment);

        if (expectedLoss > 0) {
            require(expectedLoss <= feeLossTolerance, "TooLossy");
            // Transferring tokens directly to the vault increases pricePerShare
            // Negating the immediate loss realized after the vault report.
            want.transferFrom(stash, address(vault), expectedLoss);
            emit OffsetLossOnFee(expectedLoss);
        }
